...
\institute{Universidade Nove de Julho - UNINOVE, PumasAI}
\date{2022}
...
\tikzset{
	declare function={
            ...
            laplace(\b)=(1/2*\b)*exp(-abs(x)/\b);%
            hs(\l,\t)=gaussian(0,sqrt(\l^2*\t^2));%
            f2exponential(\l)=\l*exp(-\l*(1/x-1));%
            shrinkagelaplace(\r)=f2exponential(\r)*f2jacobian(x);%
...
\include{10-Sparse_Regression}
\include{11-Hierarchical_Models}
\include{12-MCMC}
\include{13-Model_Comparison}
