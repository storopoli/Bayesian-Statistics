...
#slide(title: "All Models Are Wrong")[
  #side-by-side(columns: (4fr, 1fr))[
    #align(center + horizon)[#quote(block: true, attribution: [George Box @boxScienceStatistics1976])[
...
#slide(title: "Prior Predictive Check")[
  #text(size: 18pt)[
    We need to make sure that the posterior distribution of $bold(y)$,
    namely $bold(tilde(y))$,
    can capture all the nuances of the real distribution density/mass of $bold(y)$.

    #v(1em)

    This procedure is called *posterior predictive check*,
    and it is generally carried on by a visual inspection #footnote[
        we also perform mathematical/exact inspections,
        see the section on _Model Comparison_.]
    of the real density/mass of $bold(y)$ against generated samples
    of $bold(y)$ by the Bayesian model.
