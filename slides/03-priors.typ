...
#slide(title: "Recommended References")[
  ...
  - #cite(<gelman2020regression>, form: "prose"):
      ...
      - Chapter 9, Section 9.5: Uniform, weakly informative,
        and informative priors in regression
...
#slide(title: "Prior Probability")[
	...
	as the prior probability $P(θ)$, often just prior:
...
#slide(title: "The Subjectivity of the Prior")[
  - Many critics to Bayesian statistics are due the subjectivity
    in eliciting priors probability on certain hypothesis or model parameter's
    values.
  - Subjectivity is something unwanted in the ideal picture of the
    scientist and the scientific method.
  - Anything that involves human action will never be free from subjectivity.
    We have subjectivity in everything and science is #text(fill: julia-red)[no] exception.
  - The creative and deductive process of theory and hypotheses formulations
    is *not* objective.
  - Frequentist statistics, which bans the use of prior probabilities
    is also subjective, since there is *A LOT* of subjectivity in
    choosing which model and likelihood function
    @jaynesProbabilityTheoryLogic2003 @vandeschootBayesianStatisticsModelling2021.
]

#slide(title: "How to Incorporate Subjectivity")[
  - Bayesian statistics *embraces* subjectivity while
    frequentist statistics *bans* it.

  - For Bayesian statistics, *subjectivity guides our inferences*
    and leads to more robust and reliable models that can assist in
    decision making.

  - Whereas, for frequentist statistics, *subjectivity is a taboo*
    and all inferences should be objective,
    even if it resorts to *hiding and omitting model assumptions*.

  - Bayesian statistics also has assumptions and subjectivity,
    but these are *declared and formalized*
]

#slide(title: "Types of Priors")[
  In general, we can have 3 types of priors in a Bayesian approach
  @gelman2013bayesian @mcelreath2020statistical @vandeschootBayesianStatisticsModelling2021:
  - *uniform (flat)*: not recommended.

  - *weakly informative*: small amounts of real-world information
...
#slide(title: "Weakly Uninformative Prior")[
  ...
  something centered in $0$ with standard deviation of $1$ #footnote[
      ...
      transforming all variables into $μ=0$ and $σ=1$.]:

  - $θ tilde "Normal"(0, 1)$ (Andrew Gelman's preferred choice
    #footnote[see more about prior choices in the
    #link("https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations")[Stan's GitHub wiki].] <fn-priors-1>)
